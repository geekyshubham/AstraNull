...
Latest local-staging verification (2026-07-04): `npm test` 2163 passing; `npm run staging:local:attest` produces 31/31 release-evidence kinds and contract-valid `staging_e2e_matrix` (`overall_status=passed`). `npm run release:staging-attestation:local` reports profile inventory complete; `npm run release:gap-audit:local` now fails closed while the release-plan promotion table still has 13 `Open` external gates. This is internal SOC metadata evidence from the local Docker Compose stack — not customer-facing hosted production promotion. **Deferred (operational config):** enterprise IdP, real notification credentials, hosted KMS/HSM, customer domain edge deployment.
