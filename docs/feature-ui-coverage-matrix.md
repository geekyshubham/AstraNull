...
| (new) `/targets` | **Add** | First-class tenant inventory directly below Target groups; real `GET /v1/targets` data, filters, verification/eligibility/source context, single-domain intake, and declaration removal with evidence retained |
...
| `/targets` | Implemented; browser rerun pending | Tenant-scoped `GET /v1/targets` drives hostname/IP, group/environment, verification provenance, bounded-test eligibility/reason, source/provider context, and added time. Search/filters are client-side; single-domain add and confirmed remove use the existing nested target APIs and preserve retained evidence. | Rerun desktop/tablet/mobile browser matrix for add/remove, dark/light themes, and horizontal overflow. Real provider polling remains an external connector-worker boundary. |
