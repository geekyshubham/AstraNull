...
The control plane enforces an in-process fixed-window limiter keyed by the connection’s remote address (`req.socket.remoteAddress`). By default, inbound `X-Forwarded-For` and `X-Real-IP` are **not** trusted, so a direct client cannot spoof those headers to evade per-client limits.

Behind a load balancer that default has a cost: every connection presents the balancer's address, so all callers share a single bucket and one noisy tenant can spend the limit for everybody. Set `ASTRANULL_TRUST_PROXY_HEADERS=1` when the API sits behind a **trusted reverse proxy or load balancer** that appends the real client address, and set `ASTRANULL_TRUSTED_PROXY_HOPS` to the number of such proxies (default `1`).

The hop count matters for correctness, not just tuning. Conforming proxies **append** the peer they received from, so a caller that sends its own `X-Forwarded-For` ends up leftmost in the chain and the entries your own infrastructure wrote are on the right. The client address is therefore read `ASTRANULL_TRUSTED_PROXY_HOPS` entries from the right. Reading the leftmost entry instead would let a caller choose its own bucket key and mint a fresh allowance per request — worse than one shared bucket, which at least still counts. Setting the hop count too low degrades to that shared bucket (unfair, still not spoofable); setting it too high reads caller-supplied entries and **is** spoofable, so under-count when unsure. A chain shorter than the configured hop count falls back to the socket address rather than reading further left. `X-Real-IP` is never consulted, even with the flag on: it holds a single value with no positional information, so an edge-written value cannot be distinguished from one the caller supplied. Exceeded requests receive HTTP `429` with `error: rate_limited` and a `Retry-After` header. Stale per-client buckets are pruned as windows advance so long-lived processes do not accumulate unbounded keys. Liveness (`/health`), readiness (`/ready`), static assets, and `/metrics` are not subject to this API limiter.
