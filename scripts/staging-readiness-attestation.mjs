...
export function isSampleOrRehearsalReleaseId(releaseId) {
  ...
  if (normalized.startsWith('rel-staging-sim')) return true;
  if (normalized.includes('rehearsal')) return true;
  return false;
}

/**
 * Environments that only ever produce simulated evidence.
 *
 * `staging-sim` evidence is generated by simulators rather than collected from a running
 * stack, so it is rehearsal material and cannot support production readiness. This is
 * deliberately a simulation denylist rather than a promotion-environment allowlist:
 * `release:staging-attestation:local` legitimately attests `local-staging` bundles.
 */
const SIMULATED_EVIDENCE_ENVIRONMENTS = new Set(['staging-sim']);

function isSimulatedEvidenceEnvironment(environment) {
  if (typeof environment !== 'string') return false;
  return SIMULATED_EVIDENCE_ENVIRONMENTS.has(environment.trim().toLowerCase());
}
...
export function isRehearsalOrSampleEvidenceInput(input = {}) {
  // Accept both spellings: callers pass `rehearsal_only` (bundle envelope shape) and
  // `rehearsalOnly` (CLI-normalized shape). Reading only the snake_case key dropped the
  // flag from the attestation CLI path.
  if (input.rehearsal_only === true || input.rehearsalOnly === true) return true;
  const topReleaseId = input.releaseId ?? input.release_id ?? null;
  if (isSampleOrRehearsalReleaseId(topReleaseId)) return true;
  if (isSimulatedEvidenceEnvironment(input.environment)) return true;
  ...
  for (const record of records) {
    ...
    // Simulated evidence stays rehearsal material even when the bundle envelope
    // claims a real environment.
    if (isSimulatedEvidenceEnvironment(record?.environment)) return true;
    if (isSimulatedEvidenceEnvironment(record?.evidence?.environment)) return true;
  }
  return false;
}
