...
describe('developer test-policy scheduling', () => {
  ...
  it('rejects an incompatible effective target kind exactly as run start does before persistence', () => {
    freshStore();
    const body = {
      target_group_id: 'tg_1',
      target_id: 'tgt_1',
    ...
    const policyRejection = createTestPolicy(CTX, body);
    const runRejection = startTestRun(CTX, body);

    assert.deepEqual(policyRejection, runRejection);
    assert.deepEqual(policyRejection, {
    ...
    assert.equal(getStore().testPolicies.length, 0);
  });
  ...
  it('re-leases an expired occurrence after a start claim for idempotent recovery', () => {
    freshStore();
    const policy = createTestPolicy(CTX, {
      target_group_id: 'tg_1', target_id: 'tgt_1', check_id: CHECK_ID, cadence: 'daily',
    }, { now: '2026-06-01T00:00:00.000Z' });
    const raw = getStore().testPolicies.find((row) => row.id === policy.id);
    raw.next_run_at = '2026-06-02T00:00:00.000Z';
    ...
    const results = await dispatchDueTestPolicies(CTX, {
      ...
      async startTestRun(callCtx, body, runtimeConfig, trusted) {
        ...
        const run = { id: 'run_scheduled_exact', tenant_id: CTX.tenantId,
          target_group_id: body.target_group_id, target_id: body.target_id, check_id: body.check_id,
  ...
  it('disables a legacy due policy without target_id before any run dispatch', async () => {
    freshStore();
    const policy = createTestPolicy(CTX, {
      target_group_id: 'tg_1', target_id: 'tgt_1', check_id: CHECK_ID, cadence: 'daily',
    }, { now: '2026-06-01T00:00:00.000Z' });
    const legacy = getStore().testPolicies.find((row) => row.id === policy.id);
    legacy.target_id = null;
    legacy.next_run_at = '2026-06-02T00:00:00.000Z';
    let startCalls = 0;

    const results = await dispatchDueTestPolicies(CTX, {
      workerId: 'scheduler-legacy', now: '2026-06-02T00:00:01.000Z', leaseMs: 60_000,
      async startTestRun() {
        startCalls += 1;
        return { run: { id: 'must_not_exist' } };
      },
    });
    ...
    assert.equal(legacy.next_run_at, null);
    assert.equal(getStore().testPolicyDispatches.length, 0);
  });

  it('fails closed for event-driven create and update without a durable consumer', () => {
    freshStore();
    const rejected = createTestPolicy(CTX, {
      target_group_id: 'tg_1', target_id: 'tgt_1', check_id: CHECK_ID, cadence: 'event_driven',
      event_trigger: { type: 'finding.created' },
    });
    assert.equal(rejected.error, 'unsupported_policy_cadence');

    const policy = createTestPolicy(CTX, {
      target_group_id: 'tg_1', target_id: 'tgt_1', check_id: CHECK_ID, cadence: 'manual',
    });
